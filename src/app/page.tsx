...
export default function MapComponent({
  amenities, center, selectedId, onSelect, onOpenDrawer, onAddToItinerary,
}: MapProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        placesServiceRef.current = new gm.places.PlacesService(mapInstance.current);
        setMapLoaded(true);
  ...
  const fetchNearbyPlaces = useCallback((type: NearbyType) => {
    if (!placesServiceRef.current || !mapInstance.current) return;
    console.log('[Nearby] fetching', type);
    ...
    placesServiceRef.current.nearbySearch(
      {
        location: mapCenter,
        radius: 5000,
        // Use types array - singular 'type' is deprecated in newer Places API
        types: [NEARBY_TYPES[type].googleType],
      },
      (results: any[], status: any) => {
        const gm = (window as any).google.maps;
        console.log('[Nearby] status:', status, 'results:', results?.length);
        setLoadingNearby(prev => { const n = new Set(prev); n.delete(type); return n; });
        if (status !== gm.places.PlacesServiceStatus.OK || !results) {
          console.warn('[Nearby] fetch failed:', status);
          return;
        }
        const places: NearbyPlace[] = results.slice(0, 20).map((p: any) => ({
          id: p.place_id,
          name: p.name,
          category: type,
          address: p.vicinity || '',
        ...
        console.log('[Nearby] setting', places.length, 'places for', type);
        setNearbyPlaces(prev => [...prev.filter(x => x.category !== type), ...places]);
      }
    );
  }, []);

  // Toggle nearby type - keep fetch outside of state updater to avoid race condition
  const toggleNearbyType = useCallback((type: NearbyType) => {
    const isCurrentlyActive = activeNearbyTypes.has(type);
    if (isCurrentlyActive) {
      setActiveNearbyTypes(prev => { const n = new Set(prev); n.delete(type); return n; });
    } else {
      setActiveNearbyTypes(prev => new Set([...prev, type]));
      if (!nearbyPlaces.some(p => p.category === type)) {
        fetchNearbyPlaces(type);
      }
    }
  }, [activeNearbyTypes, nearbyPlaces, fetchNearbyPlaces]);
  ...
  <div style="padding:12px 14px;">
    ...
    <div style="display:flex;gap:5px;">
      <button onclick="window.__addNearbyToItinerary && window.__addNearbyToItinerary('${place.id}')"
        style="flex:1;padding:6px 0;background:#2563eb;color:white;border:none;border-radius:7px;font-size:10px;font-weight:600;cursor:pointer;">+ Add to Itinerary</button>
